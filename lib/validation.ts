/**
 * Security and validation helpers for API routes.
 * - UUID format validation to prevent path/injection abuse
 * - Max lengths and array caps to prevent DoS and storage abuse
 * - PATCH allowlist so clients cannot overwrite id/createdAt
 */

const UUID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;

/** Max length for single-line / short text fields. */
export const MAX_STRING_LENGTH = 2_000;

/** Max length for long text (notes, jdRaw). */
export const MAX_LONG_TEXT_LENGTH = 100_000;

/** Max length for parse JD input (matches openai-parse limit). */
export const MAX_JD_PARSE_LENGTH = 60_000;

/** Max items in techStack / collaborationTools arrays. */
export const MAX_ARRAY_ITEMS = 200;

/** Max ids in bulk delete. */
export const MAX_BULK_IDS = 200;

/** Max request body size (bytes) for JSON. */
...
/** Trims and caps a string. Returns undefined if empty after trim. */
...
/** Validates and caps array of strings. */
...
/**
 * Allowed keys for PATCH /api/jobs/[id]. Prevents overwriting id, createdAt, updatedAt.
 */
...
/** Picks only allowed keys from a PATCH body. */
...
/** Long-text keys that get MAX_LONG_TEXT_LENGTH cap. */
const PATCH_LONG_TEXT_KEYS = new Set(["jdRaw", "notes"]);
/** Keys that are string arrays (techStack, collaborationTools). */
const PATCH_ARRAY_KEYS = new Set(["techStack", "collaborationTools"]);

/** Applies length/array caps to sanitized PATCH body values. */
